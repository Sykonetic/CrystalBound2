...
const CLASS_CFG = {
  warrior:{ hp:150, mp:30, speed:115, rollCD:1.0, iframe:0.45 },
  ranger: { hp:120, mp:40, speed:130, rollCD:0.8, iframe:0.40 },
  mage:   { hp:100, mp:60, speed:115, rollCD:0.9, iframe:0.40 },
  rogue:  { hp:110, mp:35, speed:140, rollCD:0.6, iframe:0.35 },
  cleric: { hp:130, mp:70, speed:115, rollCD:0.9, iframe:0.40 } // healer
};
...
export class World{
  constructor(game){
    ...
    this.spawnInitial();
    this.boss = { x: this.w*0.7, y: this.h*0.3, hp: 800, cd: 2.2 };
  }
  ...
  update(dt){
    // boss telegraphs (reduced damage so no 1-shots)
    if(this.boss){
      this.boss.cd -= dt;
      if(this.boss.cd<=0){
        if(Math.random()<0.5){
          const t={type:'circle', x:this.player.x, y:this.player.y, r:80, ttl:1.0, dmg:20};
          ...
          const t={type:'line', x:this.boss.x, y:this.boss.y, dir, len:220, ttl:1.0, dmg:25};
          this.telegraphs.push(t);
          t.fire = ()=>{
    ...
    for(let i=this.enemies.length-1;i>=0;i--){
      ...
      if(Math.hypot(e.x-this.player.x, e.y-this.player.y)<14){
        if(!e._cd){ e._cd=0.6; this.player.hit(5, this); } // 5 dmg, with their own cd
      }
      if(e._cd) e._cd-=dt;
  ...
  draw(ctx){
    ...
    for(let j=0;j<this.gh;j++){
      for(let i=0;i<this.gw;i++){ if(this.map[j*this.gw+i]){ ctx.fillStyle='#1b2135'; ctx.fillRect(i*this.tile,j*this.tile,this.tile,this.tile); } }
    }
    // telegraphs
...
export class Player{
  constructor(world){
    ...
    this.applyClass(world);
    this.dir = 0;
    this.iframe=0; this.rollCD=0;
  ...
  attack(world){
    const arc=Math.PI/2, reach=36, dmg=16;
    let hits=0;
  ...
  update(dt, world){
    ...
    document.getElementById('hpbar').style.width=(this.hp/this.maxhp*100).toFixed(1)+'%';
    document.getElementById('mpbar').style.width=(this.mp/this.maxmp*100).toFixed(1)+'%';
...
function worldFloat(p, text, color){ const el=document.getElementById('log'); /* keep log/use floaters via world */ }
